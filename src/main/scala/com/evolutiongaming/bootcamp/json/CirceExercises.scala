...
import Models._
import com.evolutiongaming.bootcamp.json.CirceExercises.manual.manuallyDecodedAlbum
import com.evolutiongaming.bootcamp.json.CirceExercises.semiauto.{Album, Song, decodedAlbum}
...
object CirceExercises {
  ...
  object basics {
     ...
     */
    val title: Json = Json.fromString("The Matrix")
    val year: Json = Json.fromInt(1999)
    val actors: Json = Json.arr(
      Json.fromString("Keanu Reeves"),
      Json.fromString("Carrie-Anne Moss"),
      Json.fromString("Laurence Fishburne")
    )
    actors.spaces2
    val isRatedR: Json = Json.fromBoolean(true)
    ...
    lazy val jMatrix: Json = Json.obj(
      ...
      "actors" -> Json.arr(
        "Keanu Reeves".asJson,
        "Carrie-Anne Moss".asJson,
        "Laurence Fishburne".asJson
      ),
    ...
    println(killersOnTourJson.spaces2)
  }
  ...
  object semiauto {
    ...
    implicit val gigCodec = deriveCodec[Gig]
    ...
    implicit val albumCodec: Codec[Album] = deriveCodec[Album]

    private val album = Album("The Album Title", 2021, Seq(Song("Song1", 180), Song("Song2", 210)))
    lazy val albumJson: Json = album.asJson

    // Decoding back
    val decodedAlbum: Either[io.circe.Error, Album] = albumJson.as[Album]
  }
  ...
  object manual {
    ...
    implicit val albumEncoder: Encoder[Album] = Encoder.instance { album =>
      Json.obj(
        "title" -> Json.fromString(album.title),
        "year" -> Json.fromInt(album.year),
        "songs" -> album.songs.asJson
      )
    }
    ...
    implicit val albumDecoder: Decoder[Album] =
      Decoder.instance { cursor =>
        for {
          title <- cursor.downField("title").as[String]
          year <- cursor.downField("year").as[Int]
          songs <- cursor.downField("songs").as[Seq[Song]]
        } yield Album(title, year, songs)
      }
    private val album = Album("The Album Title", 2021, Seq(Song("Song1", 180), Song("Song2", 210)))

    val albumJson: Json = album.asJson
    val manuallyDecodedAlbum: Either[Error, Album] = decode[Album](albumJson.noSpaces)
  }

  def main(args: Array[String]): Unit = {
    println(manuallyDecodedAlbum)
  }
  ...
  object custom1 {
    ...
    implicit val decodeInstant: Decoder[Instant] = Decoder.decodeString.emap { str =>
      Either
        ...
        .leftMap(err => "Instant: " + err.getMessage)
    }
    ...
    implicit lazy val decodeYear: Decoder[Year] = Decoder.decodeInt.emap { intValue =>
      Either.catchNonFatal(Year.of(intValue)).left.map(err => s"Year: ${err.getMessage}")
    }
  }
  ...
  object adt {
    ...
    implicit val localDateDecoder: Decoder[LocalDate] =
      Decoder.decodeString.emap { str =>
        Either.catchNonFatal(LocalDate.parse(str)).left.map(err => s"LocalDate: ${err.getMessage}")
      }

    // Step 4: Use deriveEnumerationCodec for Enumerations
    implicit val musicianKindCodec: Codec[MusicianKind] = deriveEnumerationCodec[MusicianKind]

    // Step 5: Create Codecs for Gig, Musician, Band, and SoloMusician
    implicit val gigCodec: Codec[Gig] = deriveCodec[Gig]
    implicit val musicianCodec: Codec[Musician] = deriveCodec[Musician]
    implicit val bandCodec: Codec[Band] = deriveCodec[Band]
    implicit val soloMusicianCodec: Codec[SoloMusician] = deriveCodec[SoloMusician]
    implicit val artistCodec: Codec[Artist] = deriveConfiguredCodec[Artist]

    val artists: Seq[Artist] = Seq(theKillers, ye)
