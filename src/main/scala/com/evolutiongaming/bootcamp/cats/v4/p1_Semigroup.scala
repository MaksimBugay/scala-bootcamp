...
object p1_Semigroup {
   ...
   */
  def semigroupNel: Semigroup[NonEmptyList[Int]] = new Semigroup[NonEmptyList[Int]] {
    override def combine(x: NonEmptyList[Int], y: NonEmptyList[Int]): NonEmptyList[Int] =
      x ++ y.toList
  }

  /** Ex 1.3 implement a semigroup  for Map via concatenation
   */
  def semigroupMap: Semigroup[Map[Int, Int]] = new Semigroup[Map[Int, Int]] {
    override def combine(x: Map[Int, Int], y: Map[Int, Int]): Map[Int, Int] =
      x ++ y // `++` in Scala prioritizes the second map's values for overlapping keys
  }
  /*def semigroupMap: Semigroup[Map[Int, Int]] = new Semigroup[Map[Int, Int]] {
    override def combine(x: Map[Int, Int], y: Map[Int, Int]): Map[Int, Int] =
      x ++ y.map { case (k, v) => k -> x.get(k).map(Semigroup[Int].combine(_, v)).getOrElse(v) }
  }*/
