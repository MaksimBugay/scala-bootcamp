...
object Basics {
  ...
  val helloFunction: String => String = helloMethod
  ...
  val stringLength: String => Int = (s: String) => {
    val l : Option[Int] = Some(s.length)
    l.getOrElse(0)
  }
  ...
  def power(n: Byte): Int => Long = { x: Int =>
    // implement here
    val b : Double = x.toDouble
    val extent :Double = n.toDouble
    Math.pow(b, extent).round
  }
