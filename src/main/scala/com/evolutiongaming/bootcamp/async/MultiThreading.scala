...
import java.util.concurrent.{BlockingQueue, ConcurrentLinkedQueue, CountDownLatch, LinkedBlockingDeque, LinkedBlockingQueue}
...
object Volatility1 extends App {
  ...
  val done = new CountDownLatch(20)

  private val incFunction: Int => Int = _ + 1
  private val decFunction: Int => Int = _ - 1

  for (_ <- 0 until 10)
    new Thread(() => {
      for (_ <- 0 until 1_000_000)
        modifyThreadSafe(v, incFunction, lock)
  ...
  for (_ <- 0 until 10)
    new Thread(() => {
      for (_ <- 0 until 1_000_000)
        modifyThreadSafe(v, decFunction, lock)
      println(s"${Thread.currentThread().getName} done")
      done.countDown()
    }).start()
  ...
  def modifyThreadSafe(value: Var, modification: Int => Int, lock: ReentrantLock): Unit = {
    try {
      lock.lock()
      modification.apply(value.value)
    } finally {
      lock.unlock()
    }
  }
...
object ComplexState extends App {
  class WordCounter {
    ...
    val wordBuffer: ArrayBuffer[String] = mutable.ArrayBuffer.empty[String]
